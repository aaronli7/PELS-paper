...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 17:32:42 2020
...
data_path_DL = './w10_dataset_new/fault_detection/'
...
for num_of_training in range(KFOLD):
    ...
    dtree_f1 = sklearn.metrics.f1_score(ml_Y_test, dtree_pred, average='macro')
    dtree_precision = sklearn.metrics.precision_score(ml_Y_test, dtree_pred, average='macro')
    dtree_recall = sklearn.metrics.recall_score(ml_Y_test, dtree_pred, average='macro')
    dtree_acc = sklearn.metrics.accuracy_score(ml_Y_test, dtree_pred)
    
    svm_f1 = sklearn.metrics.f1_score(ml_Y_test, svm_pred, average='macro')
    svm_precision = sklearn.metrics.precision_score(ml_Y_test, svm_pred, average='macro')
    svm_recall = sklearn.metrics.recall_score(ml_Y_test, svm_pred, average='macro')
    svm_acc = sklearn.metrics.accuracy_score(ml_Y_test, svm_pred)
    
    knn_f1 = sklearn.metrics.f1_score(ml_Y_test, knn_pred, average='macro')
    knn_precision = sklearn.metrics.precision_score(ml_Y_test, knn_pred, average='macro')
    knn_recall = sklearn.metrics.recall_score(ml_Y_test, knn_pred, average='macro')
    knn_acc = sklearn.metrics.accuracy_score(ml_Y_test, svm_pred)

    # for ROC curve
    svm_test_y = label_binarize(ml_Y_test, classes=[0, 1])
    svm_pred_y = label_binarize(svm_pred, classes=[0, 1])
    svm_fpr, svm_tpr, _ = roc_curve(svm_test_y.ravel(), svm_pred_y.ravel())
    svm_roc_auc = sklearn.metrics.auc(svm_fpr, svm_tpr)
    
    knn_test_y = label_binarize(ml_Y_test, classes=[0, 1])
    knn_pred_y = label_binarize(knn_pred, classes=[0, 1])
    knn_fpr, knn_tpr, _ = roc_curve(knn_test_y.ravel(), knn_pred_y.ravel())
    knn_roc_auc = sklearn.metrics.auc(knn_fpr, knn_tpr)
    
    dtree_test_y = label_binarize(ml_Y_test, classes=[0, 1])
    dtree_pred_y = label_binarize(dtree_pred, classes=[0, 1])
